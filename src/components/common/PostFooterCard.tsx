...
import '../../styles/components/post-footer-card.scss';
    ...
    <Link to={post?.fields?.slug}>
      <div className="post-footer-card">
        <div className="post-footer-card__card">
          ...
          {thumbnail ? (
            <div className="next" dangerouslySetInnerHTML={{ __html: thumbnail }} />
          ) : (
            <div
              className={`next no-thumbnail theme-color-${Math.floor(Math.random() * themeSize + themeStartNumber)}`}
            >
              <span>{title}</span>
            </div>
          )}
